...
($parent) = $schema->resultset('Demo')->search({id => 10,});
...
($parent) = $schema->resultset('Demo')->search({id => 8,});
$success = $parent->insert_left_child($child);
...
($parent) = $schema->resultset('Demo')->search({id => 9,});
...
for my $test ([2,1],[3,2],[4,2],[6,4],[7,4],[5,2],[9,8],[10,8],[8,1]) {
    my $child_id    = $test->[0];
    my $parent_id   = $test->[1];
    my $child       = $schema->resultset('Demo')->find($child_id);
...
for my $test ([2,[1]],[3,[1,2]],[4,[1,2]],[6,[1,2,4]],[7,[1,2,4]],[5,[1,2]],[9,[1,8]],[10,[1,8]],[8,[1]]) {
    ...
    # list context
    $child          = $schema->resultset('Demo')->find($child_id);
...
for my $id (3,6,7,5,9,10) {
    $parent = $schema->resultset('Demo')->find($id);
    # list context
...
for my $test ([1,[2,8]],[2,[3,4,5]],[4,[6,7]],[8,[9,10]]) {
    ...
    # list context
    $parent         = $schema->resultset('Demo')->find($parent_id);
    my @children    = $parent->children;
    my $index = 0;
...
sub create_tree {
    ...
    my ($root) = $schema->resultset('Demo')->search({
        left_extent     => 1,
    });
    return $root;
}
